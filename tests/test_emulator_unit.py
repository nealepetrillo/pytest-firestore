...
class TestEmulatorInfo:
    def test_host_port_property(self) -> None:
...
class TestFindFreePort:
    def test_returns_port_from_socket(self) -> None:
        mock_sock = MagicMock()
...
class TestWaitForPort:
    ...
    def test_success_after_retries(self) -> None:
        ...
        # monotonic: 0 (deadline calc), 0 (first check),
        # 0.3 (second check), 0.6 (third check)
        with (
            patch("time.monotonic", side_effect=[0.0, 0.0, 0.3, 0.6]),
            patch("time.sleep"),
            patch(
                "pytest_firestore._emulator.socket.create_connection",
    ...
    def test_timeout_raises(self) -> None:
        ...
        with (
            patch("time.monotonic", side_effect=[0.0, 0.0, 6.0]),
            patch("time.sleep"),
            patch(
                "pytest_firestore._emulator.socket.create_connection",
                side_effect=OSError,
...
class TestFirestoreEmulatorStandalone:
    ...
    def test_info_before_start_raises(self) -> None:
        em = FirestoreEmulator()
...
class TestFirestoreEmulatorShared:
    ...
    def test_first_worker_starts(self, tmp_path: Path) -> None:
        with (
            patch.object(FirestoreEmulator, "_launch") as mock_launch,
            patch("pytest_firestore._emulator._wait_for_port"),
        ):
    ...
    def test_second_worker_joins(self, tmp_path: Path) -> None:
        self._write_state(
    ...
    def test_stale_state_restarts(self, tmp_path: Path) -> None:
        self._write_state(
            tmp_path / "firestore_emulator.json",
            {
                "pid": 999,
    ...
    def test_stop_decrements(self, tmp_path: Path) -> None:
        self._write_state(
            tmp_path / "firestore_emulator.json",
            {
                "pid": 200,
    ...
    def test_stop_last_worker_terminates(self, tmp_path: Path) -> None:
    ...
    def test_stop_no_state_file(self, tmp_path: Path) -> None:
        em = FirestoreEmulator(shared_dir=tmp_path)
